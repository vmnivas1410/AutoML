...
st.set_page_config(page_title='AutoML for Classification',
    layout='wide')
#---------------------------------#
# Model building
def build_model(df):
...
[ Developed by: Dinesh Kumar S, Nivas V M & Dr. Kanish T C ]
...
if uploaded_file is not None:
    ...
    if st.button('Press to use Example Dataset'):        
        ...
        X = pd.DataFrame(dataset.data, columns=dataset.feature_names).loc[:] # FOR TESTING PURPOSE, COMMENT THIS OUT FOR PRODUCTION
        Y = pd.Series(dataset.target, name='response').loc[:] # FOR TESTING PURPOSE, COMMENT THIS OUT FOR PRODUCTION
        df = pd.concat( [X,Y], axis=1 )

        st.markdown('The Breast Cancer dataset is used as the example.')
        st.write(df.head(600))
